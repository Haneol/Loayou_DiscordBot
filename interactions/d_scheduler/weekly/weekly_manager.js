...
const { weekly_row } = require("./weekly_row");

// weekly main
// embed, row

async function weekly_manager (interaction) {
    ...
    const row2 = await weekly_row(user, default_character);

    await interaction.reply({components: row_arr.concat(row, row2), embeds: [embed], ephemeral: true});
}

async function weekly_manager_update (interaction) {
    const user = await data.getActive(interaction.user.id);
    const characters = Object.keys(user);
    const id = interaction.user.id;
